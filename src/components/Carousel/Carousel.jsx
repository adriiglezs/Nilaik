...
import style from './Carousel.module.css';
import cart from '../../assets/cart.svg';
import slickArrow from '../../assets/user.svg';

function CarouselAdidasBi() {
  const PrevArrow = ({ onClick }) => (
    <div className={style.prevArrow} onClick={onClick}>
      <img src={slickArrow} alt="Previous Arrow" />
    </div>
  );

  const NextArrow = ({ onClick }) => (
    <div className={style.nextArrow} onClick={onClick}>
      <img src={slickArrow} alt="Next Arrow" />
    </div>
  );

  return (<>
    <h1 className={style.encabezado}>Adidas Bicolor</h1>
    <Carousel show={4.5} slide={2} transition={0.5} infinite
      responsive={[{
        breakpoint: 1024,
        settings: { slidesToShow: 3 }
      },
